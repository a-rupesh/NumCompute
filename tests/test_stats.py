import numpy as np
from stats import StreamingStats, mean, median, std, min, max, histogram, quantile
...
def streaming_stats_test():
    ...
    welford.update(x)
    assert welford.mean == np.mean(x)
    assert welford.std == np.std(x)
    assert welford.min == np.min(x)
    assert welford.max == np.max(x)
    ...
    welford.update(x)
    welford.update(y)
    assert welford.mean == np.mean(arr)
    assert welford.std == np.std(arr)
    assert welford.min == np.min(arr)
    assert welford.max == np.max(arr)
    ...
    welford.update(x)
    welford.update(y)
    assert (welford.mean == np.mean(arr, axis = 0)).all()
    assert (welford.std == np.std(arr, axis = 0)).all()
    assert (welford.min == np.min(arr, axis = 0)).all()
    assert (welford.max == np.max(arr, axis = 0)).all()

    # Test 4
    x = np.array([[1, 2], [3, 4]])
    y = np.array([5, 6, 7, 8, 9, 10])
    arr = np.concatenate((x.flatten(), y))
    welford = StreamingStats()
    welford.update(x)
    welford.update(y)
    assert welford.mean == np.mean(arr)
